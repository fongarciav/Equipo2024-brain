...
class EnterIntersectionStrategy(SignStrategy):
    ...
    def __init__(self, controller, lock, cooldown=10.0, min_confidence=0.6, activation_distance=0.1):
    ...
    def execute(self, detection: dict) -> bool:
        ...
        try:
            ...
            FORWARD_ANGLE = 105
            RIGHT_TURN_ANGLE = 60 # Close to 50 (hard right)
            SPEED = 15.0
            ...
            self.controller.command_sender.send_steering_command(FORWARD_ANGLE)
            self.controller.command_sender.send_speed_command(SPEED)
            time.sleep(1.5)
            
            # Phase 2: Turn Right
            self.controller.command_sender.send_steering_command(RIGHT_TURN_ANGLE)
            time.sleep(3.5)
